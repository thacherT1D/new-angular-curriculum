...
As we start building larger applications with multiple controllers, not only is our code per controller becoming larger, but we are starting to get faced with a new problem. How do we share data, properties and methods between controllers? Our first thought might be to use `$rootScope` and pass it to each of our controllers, but this gets messy quickly. It would be really nice to have to tool to do this - and for that we use services!
...
For all of these examples, use a `factory` to create a service as they are the most common and are quite flexible. To create a service, let's make sure we are using our new layout and have a `services.js` file. The syntax to create a factory looks like this:
...
It should function (and look far better) than this:
