...
1. Create a service
1. Use the service you created
1. Use the service to make `$http` calls
...
1. Attach properties (both state and behavior) to `this`
...
1. Use the `functionName.$inject = []` syntax to declare what you will inject
1. Add a parameter to the method that matches the name _and the order_ of the `$inject` array
...
In this section you will learn a decent pattern for making asynchronous calls from a service, namely `$http`
 calls.
...
1. Fetch a person
1. Then call `.data` on the response to get a person
1. Then get comments for the person
1. Then call `.data` on the response to get the comments
